...
self.addEventListener("push", (event) => {
  ...
  // meldingencentrum (zie notifyUser); het verdwijnt pas als je ze daar
  // afhandelt. De Badging API hangt in een service worker aan self.navigator,
  // niet aan self.registration. Een synchrone fout hier zou event.waitUntil
  // overslaan, en iOS trekt een push-abonnement in als een push geen melding
  // toont. Staat de app open, dan laat die zijn meldingen meteen opnieuw ophalen.
  const setBadge = async () => {
    const windows = await self.clients.matchAll({ type: "window", includeUncontrolled: true }).catch(() => []);
    for (const client of windows) client.postMessage({ type: "jehova:notifications-changed" });
