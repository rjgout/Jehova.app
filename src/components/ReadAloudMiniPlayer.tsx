      ...
      <div className="mx-auto max-w-5xl px-4 py-2 flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-xs font-bold text-brand-700 dark:text-brand-300 truncate">
            🔊 {source.title} — Vers {currentVerse?.number ?? ""}
          </p>
          <div className="flex items-center gap-2 mt-1">
            ...
            <select value={speed} onChange={(e) => setSpeed(Number(e.target.value))} className="shrink-0 bg-transparent text-xs font-bold text-slate-500 dark:text-slate-300 border-0 outline-none" aria-label="Voorleessnelheid">
              {SPEEDS.map((value) => <option key={value} value={value}>{value}×</option>)}
            </select>
          </div>
        </div>
        <button onClick={stop} className="shrink-0 w-7 h-7 rounded-full text-slate-400 dark:text-slate-500" aria-label="Voorlezen stoppen" title="Stoppen">
          ✕
        </button>
      </div>
    </div>
