...
const topicAnswer = z.object({ text, accept: z.array(text), evidence });
const dataSchemas: Record<AlleskennerItemKind, z.ZodTypeAny> = {
  ...
  TOPIC: z.object({ subject: text, answers: z.array(topicAnswer), distractors: z.array(text) }),
  PUZZLE: z.object({
  ...
  MEMORY: z.object({ title: text, passage: text, answers: z.array(topicAnswer), distractors: z.array(text) }),
};
...
export function validateAlleskennerItem(item: AlleskennerSeedItem, ctx: ValidationContext): string[] {
  ...
  const checkEvidence = (label: string, evidence: Evidence) => {
  ...
  if (item.kind === "QUESTION") {
    const { prompt, options, answer, evidence, listen } = item.data;
    ...
    if (evidence.length === 0) errors.push(`${id}: geen bronvers`);
  ...
  if (item.kind === "TOPIC") {
    const { subject, answers, distractors } = item.data;
    ...
    answers.forEach((a) => checkEvidence(id, a.evidence));
  }
  ...
  if (item.kind === "PUZZLE") {
    ...
    for (const group of groups) {
      ...
      if (group.evidence.length === 0) errors.push(`${id}/${group.answer}: geen bronvers`);
  ...
  if (item.kind === "MEMORY") {
    const { title, passage, answers, distractors } = item.data;
    ...
    for (const answer of answers) {
      checkEvidence(id, answer.evidence);
